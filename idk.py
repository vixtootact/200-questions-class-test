...
#453. UnboundLocalError: local variable 'x' referenced before assignment.
...
def outer():
    x = 20
    def inner():
        print(x)  # 20 from enclosing scope
    inner()
outer()

#456. UnboundLocalError — same reason as Q453.
...
def add_fixed(item, L=None):
    if L is None:
        L = []  # new list created fresh on every call
    L.append(item)
    return L
print(add_fixed(1))  # [1]
print(add_fixed(2))  # [2]
...
z = x.copy()
x.append(4)
print(y, z)  # [1, 2, 3, 4] [1, 2, 3]

#461.
import copy
a = [[1, 2], [3, 4]]
b = a.copy()  # shallow: outer list copied but inner lists shared
c = copy.deepcopy(a)  # deep: everything copied independently
a[0][0] = 99
print(b)  # [[99, 2], [3, 4]] — b shares inner lists with a
print(c)  # [[1, 2], [3, 4]]  — c is fully independent

#462. True (usually for short identifier-like strings).
a = "hello_world_test_123"
b = "hello_world_test_123"
print(a is b)  # True (interned) — not guaranteed but common for these patterns
...
#466. Python 3: SyntaxError — True and False are keywords, cannot be used as variable names.
